...
if (isset($_POST['username'], $_POST['mail'], $_POST['contrasena'], $_POST['Contrasena_Again'])) {
    ...
    if (empty($username) or empty($mail) or empty($contrasena) or empty($Contrasena_Again)) {
        ...
        $check_mail = "SELECT * FROM Register WHERE mail = '$mail'";
        $result_mail = $conexion->query($check_mail);

        $check_user = "SELECT * FROM Register WHERE username = '$username'";
        $result_user = $conexion->query($check_user);
        ...
        if ($result_user->num_rows > 0) {
            ...
            if ($contrasena === $Contrasena_Again) {
                $query = "INSERT INTO Register (username, mail, contrasena) VALUES ('$username', '$mail', '$contrasena')";
                if ($conexion->query($query) === TRUE) {

                    echo '<script>
                            window.location = "../views/register_view.html"
                        </script>';
